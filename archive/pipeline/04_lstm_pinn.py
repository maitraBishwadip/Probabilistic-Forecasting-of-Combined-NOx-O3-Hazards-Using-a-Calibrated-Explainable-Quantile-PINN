# -*- coding: utf-8 -*-
"""
04_lstm_pinn.py  — THE model: LSTM-PINN for compound PM2.5-O3 extremes.

A 14-day daily sequence per station -> LSTM encoder -> (with static station/location)
  - marginal exceedance heads  p_PM, p_O3
  - concentration heads ĉ_PM, ĉ_O3 (softplus >=0), regularised by a daily box-model
  - regime-indexed dependence  lift_r  (learned)
  compound prob  p_comp = clamp(p_PM * p_O3 * lift_r, 0, 1)

Physics (daily box mass balance, regime-indexed learnable params a,b,d,p,q >=0):
...
Robust eval of THIS one model: temporal hold-out (test 2016), LOSO, per-regime
PR-AUC, bootstrap CIs, lead times 24/48/72 h, isotonic calibration.
Ablations (architecture justification, NOT competing models):
  LSTM vs single-day MLP ; +/- physics ; +/- regime-lift.
Reference skill lines only: base rate, persistence.
"""
...
REGOH=[f"regime_oh{i}" for i in range(K)]
...
# standardize seq features on TRAIN-year rows
trmask=df["year"].isin([2014,2015])
mu=df.loc[trmask,SEQ].mean(); sd=df.loc[trmask,SEQ].std().replace(0,1)
dfs=df.copy()
for c in SEQ: dfs[c]=(df[c]-mu[c])/sd[c]
# per-timestep missingness flags for the two targets' pollutants
...
rows=[]
Xseq=[]; stat=[]; y24=[]; y48=[]; y72=[]; reg=[]; yr=[]; st=[]
pm_t=[]; o3_t=[]; vc=[]; pr=[]; ph=[]; pm_n=[]; o3_n=[]; pmx=[]; o3x=[]; opm=[]; oo3=[]; pers=[]
...
for s in stations:
    n=len(raw["y24"][s]); Fmat=np.stack([arr[c][s] for c in SEQF],axis=1)  # [n,F]
    for i in range(L-1,n):
        ...
        reg.append(int(raw["regime"][s][i]) if np.isfinite(raw["regime"][s][i]) else 0)
        yr.append(int(raw["year"][s][i]))
        ...
        pmx.append(raw["pm_ex_next"][s][i]); o3x.append(raw["o3_ex_next"][s][i])
        pm_n.append(raw["pm_next"][s][i]); o3_n.append(raw["o3_next"][s][i])
...
np.nan_to_num(pm_t,copy=False); np.nan_to_num(o3_t,copy=False); np.nan_to_num(vc,copy=False); np.nan_to_num(pr,copy=False); np.nan_to_num(ph,copy=False)
pmx=f(pmx); o3x=f(o3x); obm=np.isfinite(pmx).astype("float32"); obo=np.isfinite(o3x).astype("float32")
pmx=np.nan_to_num(pmx); o3x=np.nan_to_num(o3x)
cpm_n=f(pm_n)/SC["pm"]; co3_n=f(o3_n)/SC["o3"]; opm=np.isfinite(cpm_n).astype("float32"); oo3=np.isfinite(co3_n).astype("float32")
cpm_n=np.nan_to_num(cpm_n); co3_n=np.nan_to_num(co3_n); pers=np.nan_to_num(f(pers))
...
class LSTMPINN(nn.Module):
    def __init__(self,F,S,K,use_lstm=True,use_regime=True,use_lift=True):
        ...
        self.encdrop=nn.Dropout(0.35)   # curb LSTM overfitting on a 3-yr record
        self.stat=nn.Embedding(S,8)
        # skip connection: last-day raw features fed alongside the encoder state
        ...
        nl=K if (use_regime and use_lift) else 1
        self.lift=nn.Parameter(torch.zeros(nl)+0.541)
    def forward(self,x,sid,ridx):
        if self.use_lstm: o,_=self.enc(x); h=self.encdrop(o[:,-1,:])
        else: h=self.enc(x[:,-1,:])
        z=self.head(torch.cat([h,x[:,-1,:],self.stat(sid)],1))
        p_pm=torch.sigmoid(self.h_pm(z)).squeeze(-1); p_o3=torch.sigmoid(self.h_o3(z)).squeeze(-1)
        c=sp(self.c_head(z))
        if self.use_lift:
...
import copy
def fit(target="y24",use_lstm=True,use_regime=True,use_lift=True,use_phys=True,itr=None,ite=None,ep=200,bs=256,patience=25):
    yt_all={"y24":y24,"y48":y48,"y72":y72}[target]
    valid=np.isfinite(yt_all)
    if itr is None: itr=valid&np.isin(yr,[2014,2015]); ite=valid&(yr==2016)
    net=LSTMPINN(Fdim,Sdim,K,use_lstm,use_regime,use_lift)
    opt=torch.optim.Adam(net.parameters(),lr=1.5e-3,weight_decay=1e-4)
    gidx=np.where(itr)[0]
    # chrono-agnostic 85/15 val split for early stopping (fixed seed)
    rs=np.random.default_rng(1).permutation(len(gidx)); nval=max(50,int(0.15*len(gidx)))
    vsel=gidx[rs[:nval]]; idx=gidx[rs[nval:]]
    Xt=T(Xseq[idx]); sid=torch.tensor(stat[idx]); rid=torch.tensor(reg[idx])
    yt=T(yt_all[idx].astype("float32")); pmt=T(pmx[idx]); o3t=T(o3x[idx]); mbm=T(obm[idx]); mbo=T(obo[idx])
    ...
    for e in range(ep):
        ...
        for j in range(0,N,bs):
            ...
            p_pm,p_o3,c,pc=net(Xt[b],sid[b],rid[b])
            ...
            Lm=(bpm*mbm[b]).sum()/(mbm[b].sum()+1e-6)+(bo3*mbo[b]).sum()/(mbo[b].sum()+1e-6)
            Lc=focal_p(pc,yt[b])
            Lreg=((c[:,0]-cpn[b])**2*mpm[b]).sum()/(mpm[b].sum()+1e-6)+((c[:,1]-con[b])**2*mo3[b]).sum()/(mo3[b].sum()+1e-6)
            lam=min(0.5,e/60*0.5)
            Lph=((c-net.phys(cpt[b],cot[b],vct[b],prt[b],pht[b],rid[b]))**2).mean() if use_phys else torch.tensor(0.0)
            loss=Lm+Lc+0.3*Lreg+(lam*Lph if use_phys else 0.0)
...
RES={}
# ---- main model ----
net,p,yv,ptr,ytr=fit("y24",True,True,True)
...
yh=(p>=tstar).astype(int)
from sklearn.metrics import f1_score
m["F1_test"]=float(f1_score(yv,yh)); m["thr_star"]=float(tstar)
pP,rP,_=precision_recall_curve(yv,p); ok=pP>=0.3; m["recall_at_prec0.3"]=float(rP[ok].max()) if ok.any() else 0.0
iso=IsotonicRegression(out_of_bounds="clip").fit(ptr,ytr); pcal=iso.transform(p)
m["Brier_calibrated"]=float(brier_score_loss(yv,pcal))
RES["LSTM_PINN"]=m

# reference skill
RES["reference"]=dict(base_rate=float(yv.mean()),
                      persistence_PR_AUC=float(average_precision_score(yv, pers[(np.isfinite(y24))&(yr==2016)])))

# ---- ablations (architecture justification) ----
def quick(**k):
    _,pp,yy,_,_=fit("y24",ep=110,**k); return round(float(average_precision_score(yy,pp)),3), round(float(roc_auc_score(yy,pp)),3)
RES["ablation"]={}
RES["ablation"]["LSTM_PINN(full)"]=dict(PR_AUC=round(m["PR_AUC"],3),ROC_AUC=round(m["ROC_AUC"],3))
ap,ro=quick(use_lstm=False); RES["ablation"]["MLP_instead_of_LSTM"]=dict(PR_AUC=ap,ROC_AUC=ro)
ap,ro=quick(use_phys=False); RES["ablation"]["no_physics"]=dict(PR_AUC=ap,ROC_AUC=ro)
ap,ro=quick(use_lift=False); RES["ablation"]["no_regime_lift(independence)"]=dict(PR_AUC=ap,ROC_AUC=ro)
ap,ro=quick(use_regime=False); RES["ablation"]["no_regime_params"]=dict(PR_AUC=ap,ROC_AUC=ro)

# ---- lead times ----
RES["lead_times"]={"24h":round(m["PR_AUC"],3)}
for tg in ["y48","y72"]:
    _,pp,yy,_,_=fit(tg,ep=110); RES["lead_times"][tg.replace("y","")+"h"]=round(float(average_precision_score(yy,pp)),3)
...
for i in range(K):
    ...
    if y24[ite][sel].sum()>=3:
        per[names[i]]=dict(PR_AUC=round(float(average_precision_score(y24[ite][sel].astype(int),p[sel])),3),
                           n=int(sel.sum()),n_pos=int(y24[ite][sel].sum()))
RES["per_regime_PR_AUC"]=per

# ---- learned interpretable params ----
with torch.no_grad():
    RES["learned_regime_lift"]={names[i]:round(float(sp(net.lift)[i]),3) for i in range(K)}
    RES["learned_physics_by_regime"]={names[i]:dict(dilution_a=round(float(sp(net.a)[i]),3),
        wet_b=round(float(sp(net.b)[i]),3),base_loss_d=round(float(sp(net.d)[i]),3),
        o3_prod_p=round(float(sp(net.p)[i]),3),pm_emit_q=round(float(sp(net.q)[i]),3)) for i in range(K)}
...
for s in stations:
    ...
    _,pp,yy,_,_=fit("y24",itr=itr,ite=ite2,ep=90); loso.append(float(average_precision_score(yy,pp)))
...
pp,rr,_=precision_recall_curve(yv,p); plt.plot(rr,pp,label=f"LSTM-PINN (AP={m['PR_AUC']:.2f})")
plt.axhline(yv.mean(),ls=':',c='grey',label=f"base rate={yv.mean():.2f}")
plt.xlabel("recall");plt.ylabel("precision");plt.legend();plt.title("LSTM-PINN — compound 24h PR curve (test 2016)")
plt.tight_layout();plt.savefig(FIG/"pr_lstm_pinn.png",dpi=130);plt.close()
bins=np.linspace(0,1,11); idx=np.digitize(pcal,bins)-1; xs=[];ys=[]
for bn in range(10):
    sb=idx==bn
    if sb.sum()>5: xs.append(pcal[sb].mean()); ys.append(yv[sb].mean())
plt.figure(figsize=(4,4)); plt.plot([0,1],[0,1],'k--',lw=1); plt.plot(xs,ys,'o-')
plt.xlabel("predicted");plt.ylabel("observed");plt.title("LSTM-PINN reliability (calibrated)")
plt.tight_layout();plt.savefig(FIG/"reliability_lstm_pinn.png",dpi=130);plt.close()

json.dump(RES,open(ROOT/"results_lstm_pinn.json","w"),indent=2)
